...
import { userTasks } from "../../redux/feature/tasks/tasksSlice";

const MyTasks = () => {
  const { tasks, userSpecificTasks } = useSelector((state) => state.tasksSlice);
  const { name: useName } = useSelector((state) => state.userSlice);
  ...
  useEffect(() => {
    dispatch(userTasks(useName));
  }, [useName, dispatch, tasks]);
